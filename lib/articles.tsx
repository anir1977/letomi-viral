...
export interface ArticleListItem extends ArticleIndexItem {
  views: number;
  status: "Published";
}
...
const articles = articleIndex as ArticleIndexItem[];
...
function withViews(item: ArticleIndexItem): ArticleListItem {
  const views = 1500 + (hashSlug(item.slug) % 75000);
  return {
    ...item,
    views,
    status: "Published",
...
export function getAllArticles(): ArticleIndexItem[] {
  return articles;
}

export function getArticleBySlug(slug: string): ArticleIndexItem | undefined {
...
export function getTrendingArticles(limit: number = 10): ArticleListItem[] {
  return articles
    .map(withViews)
...
export function getTopArticles(limit: number = 10): ArticleListItem[] {
  return getTrendingArticles(limit);
}

export function getRelatedArticles(slug: string, limit: number = 3): ArticleIndexItem[] {
  const current = getArticleBySlug(slug);
  const pool = current
    ? articles.filter((article) => article.slug !== slug && article.category === current.category)
    : articles;

  const fallbackPool = pool.length > 0 ? pool : articles.filter((article) => article.slug !== slug);
  return fallbackPool.slice(0, limit);
}

export function searchArticles(query: string): ArticleIndexItem[] {
...
export const articleContent: Record<string, ArticleContentEntry> = {
